...
# This script executes the Python worker directly from memory.
# You can pass arguments to it just like a normal script:
# ./run_worker.sh --server ws://example.com --fake-upstream
...
async def write_ws_frame(writer: asyncio.StreamWriter, opcode: int, payload: bytes = b"", *, mask: bool) -> None:
...
class RelayClient:
    ...
    async def run_once(self) -> None:
        ...
        await self.send({"type": "hello", "worker_id": self.worker_id})
        ...
        try:
            ...
            ping.cancel()
            sender.cancel()
            for job in list(self.ws_jobs.values()): job.close()
            writer.close()

    async def connect_ws(self) -> tuple[asyncio.StreamReader, asyncio.StreamWriter]:
        split = urlsplit(self.server_url)
        host, port = split.hostname or "localhost", split.port or (443 if split.scheme == "wss" else 80)
        path = (split.path or "/env") + ("?" + split.query if split.query else "")
        ssl_context = ssl.create_default_context() if split.scheme == "wss" else None
        reader, writer = await asyncio.open_connection(host, port, ssl=ssl_context)
        sock = writer.get_extra_info("socket")
        if sock: set_tcp_nodelay(sock)
        key = make_ws_key()
        headers = [f"GET {path} HTTP/1.1", f"Host: {host}:{port}", "Upgrade: websocket", "Connection: Upgrade", f"Sec-WebSocket-Key: {key}", "Sec-WebSocket-Version: 13"]
        writer.write(("\r\n".join(headers) + "\r\n\r\n").encode("ascii"))
        await writer.drain()
        status_line = await reader.readline()
        if b"101" not in status_line: raise ConnectionError(status_line.decode())
    ...
    async def sender(self, writer: asyncio.StreamWriter) -> None:
        while True:
            ...
            async with self.writer_lock:
                await write_ws_frame(writer, 1, json.dumps(msg).encode("utf-8"), mask=True)
    ...
    def send_from_thread(self, msg: dict[str, Any]) -> None:
        if self.loop: self.loop.call_soon_threadsafe(self.outgoing.put_nowait, msg)

    async def handle_message(self, msg: dict[str, Any]) -> None:
        typ, job_id = msg.get("type"), msg.get("job_id")
        if typ == "http_request" and job_id:
            ...
            if job: job.put_frame(int(msg.get("opcode") or 1), unb64(msg.get("data")))
    ...
    def run_http_job(self, msg: dict[str, Any]) -> None:
        ...
        conn = None
        try:
            method, path = msg.get("method") or "POST", msg.get("path") or "/v1/responses"
            body, headers = unb64(msg.get("body")), filtered_headers(msg.get("headers") or [], websocket=False)
            conn = http.client.HTTPSConnection(UPSTREAM_PROXY_HOST, UPSTREAM_PROXY_PORT)
            conn.set_tunnel(UPSTREAM_HOST, UPSTREAM_PORT)
            conn.request(method, path, body=body if body else None, headers=headers)
            resp = conn.getresponse()
            h_list = [[k, v] for k, v in resp.getheaders()]
            self.send_from_thread({"type": "http_start", "job_id": job_id, "status": resp.status, "reason": resp.reason, "headers": h_list})
            while True:
    ...
    def run_fake_http_job(self, msg: dict[str, Any]) -> None:
        job_id = msg["job_id"]
        self.send_from_thread({"type": "http_start", "job_id": job_id, "status": 200, "reason": "OK", "headers": [["Content-Type", "application/json"]]})
        self.send_from_thread({"type": "http_body", "job_id": job_id, "data": b64(json.dumps({"status": "completed", "worker_id": self.worker_id}).encode())})
        self.send_from_thread({"type": "http_end", "job_id": job_id})

class WsUpstreamJob:
    def __init__(self, relay: RelayClient, msg: dict[str, Any]) -> None:
        self.relay, self.msg, self.job_id = relay, msg, msg["job_id"]
        self.outbound, self.closed = queue.Queue(), threading.Event()
        self.thread = threading.Thread(target=self.run, daemon=True)

    def start(self) -> None: self.thread.start()
    def put_frame(self, opcode: int, payload: bytes) -> None: self.outbound.put((opcode, payload))
    def close(self) -> None: self.closed.set(); self.outbound.put(None)

    def run(self) -> None:
        sock = None
        try:
            sock, resp_headers = self.connect_upstream()
            self.relay.send_from_thread({"type": "ws_open", "job_id": self.job_id, "headers": [[k, v] for k, v in resp_headers.items()]})
            while not self.closed.is_set():
                while not self.outbound.empty():
                    item = self.outbound.get_nowait()
                    if item is None: break
                    write_blocking_ws_frame(sock, item[0], item[1], mask=True)
                r, _, _ = select.select([sock], [], [], 0.1)
                if r:
                    op, py = read_blocking_ws_frame(sock)
                    self.relay.send_from_thread({"type": "ws_frame", "job_id": self.job_id, "opcode": op, "data": b64(py)})
                    if op == 8: break
            self.relay.send_from_thread({"type": "ws_close", "job_id": self.job_id, "code": 1000, "reason": ""})
        except Exception as exc:
            self.relay.send_from_thread({"type": "job_error", "job_id": self.job_id, "message": str(exc)})
        finally:
            self.relay.ws_jobs.pop(self.job_id, None)
            if sock: sock.close()

    def connect_upstream(self) -> tuple[ssl.SSLSocket, dict[str, str]]:
        raw = socket.create_connection((UPSTREAM_PROXY_HOST, UPSTREAM_PROXY_PORT))
        raw.sendall(f"CONNECT {UPSTREAM_HOST}:{UPSTREAM_PORT} HTTP/1.1\r\nHost: {UPSTREAM_HOST}:{UPSTREAM_PORT}\r\n\r\n".encode())
        while b"\r\n\r\n" not in (head := raw.recv(4096)): pass
        ...
        headers.update({"Host": f"{UPSTREAM_HOST}:{UPSTREAM_PORT}", "Upgrade": "websocket", "Connection": "Upgrade", "Sec-WebSocket-Key": key, "Sec-WebSocket-Version": "13"})
        request = f"GET {self.msg.get('path', '/v1/responses')} HTTP/1.1\r\n" + "".join(f"{k}: {v}\r\n" for k, v in headers.items()) + "\r\n"
        sock.sendall(request.encode("iso-8859-1"))
        while b"\r\n\r\n" not in (resp := sock.recv(4096)): pass
        return sock, {}
...
def main() -> None:
    ...
    parser.add_argument("--fake-stream-delay", type=float, default=0.15)
    args = parser.parse_args()
    try:
        asyncio.run(RelayClient(args).run_forever())
